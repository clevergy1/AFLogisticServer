...
Namespace SCP.BLL
    Public Class hs_amb_Profile_Tasks
#Region "constructor"
        Public Sub New()
...
#Region "methods"
        Public Shared Function Add(CronId As Integer,
            ...
            Return DataAccessHelper.GetDataAccess.hs_amb_Profile_Tasks_Add(CronId, ProfileNr, Subject, StartDate, EndDate, m_RecurrencePattern, String.Empty, yearsRepeatable)
        End Function

        Public Shared Function Del(CronId As Integer, ProfileNr As Integer) As Boolean
            If CronId <= 0 Then Return False
            If ProfileNr < 0 Then Return False
            Return DataAccessHelper.GetDataAccess.hs_amb_Profile_Tasks_Del(CronId, ProfileNr)
        End Function

        Public Shared Function List(CronId As Integer) As List(Of hs_amb_Profile_Tasks)
            If CronId <= 0 Then Return Nothing
            Return DataAccessHelper.GetDataAccess.hs_amb_Profile_Tasks_List(CronId)
        End Function

        Public Shared Function ListAll() As List(Of hs_amb_Profile_Tasks)
            Return DataAccessHelper.GetDataAccess.hs_amb_Profile_Tasks_ListAll()
        End Function

        Public Shared Function ListCurrent(CronId As Integer, selDate As Date) As List(Of hs_amb_Profile_Tasks)
            If CronId <= 0 Then Return Nothing

            Dim workList As New List(Of hs_amb_Profile_Tasks)

            Dim profileTaskList As List(Of hs_amb_Profile_Tasks) = DataAccessHelper.GetDataAccess.hs_amb_Profile_Tasks_List(CronId)
            If Not profileTaskList Is Nothing Then
                For Each _obj As hs_amb_Profile_Tasks In profileTaskList
                    ...
                    Else
                        ...
                        For Each occurrence As Occurrence In rrrule
                            If occurrence.Start.Date = selDate Then
                                Dim t1 As Date = occurrence.Start.Year & "/" & occurrence.Start.Month.ToString.PadLeft(2, "0") & "/" & occurrence.Start.Day.ToString.PadLeft(2, "0") &
                                    "T" & _obj.StartDate.Hour.ToString.PadLeft(2, "0") & ":" & _obj.StartDate.Minute.ToString.PadLeft(2, "0")
                                Dim t2 As Date = occurrence.Start.Year & "/" & occurrence.Start.Month.ToString.PadLeft(2, "0") & "/" & occurrence.Start.Day.ToString.PadLeft(2, "0") &
            ...
            If workList.Count > 0 Then
                ...
                Dim retVal As New List(Of hs_amb_Profile_Tasks)
        ...
        Public Shared Function ListByMonth(CronId As Integer, calYear As Integer, calMonth As Integer) As List(Of hs_amb_Profile_Tasks)
            ...
            For Each Day As DateTime In Enumerable.Range(0, (endDate - startDate).Days).Select(Function(i) startDate.AddDays(i))
                Dim _list As List(Of hs_amb_Profile_Tasks) = ListCurrent(CronId, Day)
                If Not _list Is Nothing Then
                    For Each _obj As hs_amb_Profile_Tasks In _list
                        retval.Add(_obj)
                    Next
                End If
            Next Day
        ...
        Public Shared Function EXListByMonth(CronId As Integer, calYear As Integer, calMonth As Integer) As List(Of hs_amb_Profile_Tasks)
            If CronId <= 0 Then Return Nothing
                ...
                For Each _obj As hs_amb_Profile_Tasks In profileTaskList
                    ...
                    If app.End >= endDate Then
                        ...
                        Else
                            ...
                            For Each occurrence As Occurrence In rrrule
                                If occurrence.Start.Date >= selDate Then
                                    Dim taskObj As New SCP.BLL.hs_amb_Profile_Tasks
        ...
        Public Shared Function ListByDates(CronId As Integer, startDate As Date, endDate As Date) As List(Of hs_amb_Profile_Tasks)
            If CronId <= 0 Then Return Nothing
        ...
        Public Shared Function Read(TaskId As Integer) As hs_amb_Profile_Tasks
            If TaskId <= 0 Then Return Nothing
            Return DataAccessHelper.GetDataAccess.hs_amb_Profile_Tasks_Read(TaskId)
        End Function

        Public Shared Function Update(TaskId As Integer,
                                      ProfileNr As Integer,
                                      Subject As String,
                                      StartDate As String,
                                      EndDate As String,
            ...
            'Dim oldTask As hs_amb_Profile_Tasks = SCP.BLL.hs_amb_Profile_Tasks.Read(TaskId)
            'If Not oldTask Is Nothing Then

            '    oldTask = Nothing
            'End If

            Return DataAccessHelper.GetDataAccess.hs_amb_Profile_Tasks_Update(TaskId, ProfileNr, Subject, StartDate, EndDate, m_RecurrencePattern, String.Empty, yearsRepeatable)
        End Function
#End Region
